...
pdf(file = "./doc/figures/str_tran_expenditure.pdf", width = 10, height = 6)
...
str.plot.name <- function(dat, lab.y, default.color.base, bw2 = TRUE) {
  ggplot(data = dat, aes(x = log(rgdpe.pc), y = value)) +
...
pdf(file = "./doc/figures/str_tran_expenditure_country_name.pdf", width = 10, height = 6)
str.plot.name(dat.cons.name, "Share of final expenditure", default.color.base) + facet_wrap(~title.alp)
dev.off()
...
str.plot.diff <- function(dat, lab.y, default.color.base, bw2 = TRUE) {
...
pdf(file = "./doc/figures/str_tran_expenditure_country_diff.pdf", width = 10, height = 6)
str.plot.diff(dat.cons.diff, "Change in the final expenditure shares from 1995 to 2018", default.color.base) + facet_wrap(~title.rom)
dev.off()
...
pdf(file = "./doc/figures/str_tran_prd.pdf", width = 10, height = 6)
...
pdf(file = "./doc/figures/str_tran_prd_country_name.pdf", width = 10, height = 6)
str.plot.name(dat.prod.name, "Share of GDP", default.color.base) + facet_wrap(~title.alp)
dev.off()

## Differences
dat.prod.diff <- add.diff(dat.prod)

pdf(file = "./doc/figures/str_tran_prd_country_diff.pdf", width = 10, height = 6)
str.plot.diff(dat.prod.diff, "Change in the GDP shares from 1995 to 2018", default.color.base) + facet_wrap(~title.alp)
dev.off()
...
writeLines(paste("For consumption, numbers of exceptions are", above.zero(dat.cons.diff, "G"), "for G,", 66 - above.zero(dat.cons.diff, "HTS"), "for HTS,", 66 - above.zero(dat.cons.diff, "BTS"), "for BTS."), "./doc/numbers/str_chg_expenditure_change_exception.txt")

writeLines(paste("For production, numbers of exceptions are", above.zero(dat.prod.diff, "G"), "for G,", 66 - above.zero(dat.prod.diff, "HTS"), "for HTS,", 66 - above.zero(dat.prod.diff, "BTS"), "for BTS."), "./doc/numbers/str_chg_prd_change_exception.txt")
...
writeLines(paste0("Services share of world total GDP is ", sum(dat.18$tot.va[dat.18$ind %in% c("hts", "bts")]) / sum(dat.18$tot.va), "."), "./doc/numbers/service_share_gdp.txt")
