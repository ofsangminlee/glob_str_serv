...
plot.frac.g <- function(res.g, num.limits, num.breaks, bw = TRUE) {
  ...
  pp <- ggplot(res.g, aes(x = `1995`, y = `2018`)) +
    ...
    xlab("Fraction of goods (1995)") +
    ylab("Fraction of goods (2018)") +
...
pdf(file = "./doc/figures/goods_fraction.pdf", width = 10, height = 6)
...
write.csv(res.g.chn.usa, "./doc/numbers/goods_fraction_chn_usa.csv", row.names = FALSE)
...
writeLines(paste("ROW share of the world total exports is", export.2018$total[export.2018$country == "ROW"] / export.2018$total[export.2018$country == "TOT"]), "./doc/numbers/row_share_exports.txt")
...
writeLines(paste0("Goods and services trade as a share of GDP in 1995 were ", tr.sh.1995.g, " and ", tr.sh.1995.s, ", respectively. For 2018, the numbers were ", tr.sh.2018.g, " and ", tr.sh.2018.s, "."), "./doc/numbers/trade_gdp_share_1995_2018.txt")
...
trd.sector.2018 <- c(
  go.tr.ind$tr[go.tr.ind$year == 2018 & go.tr.ind$ind == "G"],
  go.tr.ind$tr[go.tr.ind$year == 2018 & go.tr.ind$ind == "HTS"],
  go.tr.ind$tr[go.tr.ind$year == 2018 & go.tr.ind$ind == "BTS"]
)

writeLines(paste0("In 2018, tradedness for G, HTS, and BTS were as follows:", paste(trd.sector.2018, collapse = ", "), "."), "./doc/numbers/tradedness_sector_2018.txt")
...
writeLines(paste0("Services share of world total exports is ", num.ser.share, "."), "./doc/numbers/service_share_export.txt")
