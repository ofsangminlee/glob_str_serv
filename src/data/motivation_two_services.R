...
plot.tradedness.intermediateness <- function(dat, size.repel = 5, my.theme = NULL, var.color = "g.bts.hts", pal.color = default.color.base) {
  ggplot(dat, aes(x = intermediateness, y = tradedness)) +
    geom_point(aes(color = !!sym(var.color), shape = !!sym(var.color)), size = 3) +
...
pdf(file = "./doc/figures/int_tr_2018.pdf", width = 10, height = 6) ## _paper removed.
plot.tradedness.intermediateness(int.tr.2018)
dev.off()

## Save result in CSV
write.csv(int.tr.2018, file = "./doc/figures/int_tr_2018.csv", row.names = FALSE)
...
write.csv(motivation.num.total.trade, file = "./doc/numbers/motivation_trade_volumes.csv", row.names = FALSE)
...
write.csv(int.tr.2018.g, "./doc/figures/int_tr_2018_g.csv", row.names = FALSE)
...
write.csv(dat.aa, "./doc/numbers/fraction_acom_art.csv", row.names = FALSE)

## About 9% of BTS, 7% of HTS, 4% of S
...
## About 6% of total services trade
...
############################################
## How much of BTS and HTS are traveling? ##
############################################
...
write.csv(tr.type.sum, file = "./doc/numbers/share_type_export.csv", row.names = FALSE)
