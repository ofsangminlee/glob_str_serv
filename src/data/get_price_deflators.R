...
check.coverage <- function(dat, vars) {
  do.call("rbind", lapply(split(dat, dat$country), function(dat.country) {
...
which.missing <- function(list.dat, years.x) {
  lapply(list.dat, function(dat) {
...
writeLines(paste("Among", nrow(res.gopd.vapd), "country-sectors,", sum(res.gopd.vapd$cor.improve == "Y"), ",", sum(res.gopd.vapd$rmse.improve == "Y"), ", and", sum(res.gopd.vapd$beta.improve == "Y"), "improved based on correlation, RMSE, and regression coefficients, respectively."), "./doc/numbers/imputed_gopd_vs_vapd.txt")
