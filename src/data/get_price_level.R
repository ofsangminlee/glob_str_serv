...
common.options <- list(
  geom_text(aes(x = log(gdppc), y = log(rel.pl.2005), color = ind, label = country), alpha = 0.5),
...
pdf(file = "./doc/figures/price_levels.pdf", width = 10, height = 6) # If you want vertical plot, width 10, height = 13
ggplot(data = res) +
  common.options +
...
writeLines(paste0("ROW share of the world total GDP is ", gdp.2018$gdp[gdp.2018$country == "ROW"] / sum(gdp.2018$gdp), "."), "./doc/numbers/row_share_gdp.txt")
...
temp <- gopl[, c("country", "year", "ind", "pl")]
colnames(temp) <- c("country", "year", "sector", "gross_output_price_level")
temp <- temp[order(temp$country, temp$year, temp$sector), ]
temp$year <- as.integer(substr(temp$year, 2, nchar(temp$year)))
write.csv(temp, file = "./output/cleaned_data/gross_output_prices_66countries_1995_2018_3sectors.csv", row.names = FALSE)
