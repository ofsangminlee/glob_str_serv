...
## Load price data (ASSUMPTION: ROW's prices = those of USA, following Kehoe-Ruhl-Steinberg)
...
## Why US's sectoral net exports over year? To check specialization patterns.
## Alternative: sectoral net export / sectoral GDP.
...
pdf(file = "./doc/figures/usa_nx_sector.pdf", width = 10, height = 6)
...
## One problem with the above, you have to think about borrowing and lending choices (i.e. total net export of a country). Doing goods fractions of exports and imports could be a better idea.
...
label.data <- data.frame(
  year = last.year,
  y = c(
    res.g$f.imports[res.g$year == last.year],
...
pdf(file = "./doc/figures/usa_specialization_years.pdf", width = 10, height = 6)
...
write.csv(res.g, "./doc/figures/usa_specialization_years.csv", row.names = FALSE)
...
params.pref <- read.csv("./output/params/sub_inc_elasticity_wo_usa.csv")
...
write.csv(phis.usa.zzz, file = "./output/params/usa_row_weight_util.csv", row.names = FALSE)
...
## Load rho's
sum.rho <- read.csv("./output/params/rho_wo_usa.csv")
...
write.csv(res.usa.zzz, file = "./output/params/usa_row_alphas.csv", row.names = FALSE)
...
clean.trade <- function(dat.icio, list.inds) {
  icio.trade <- lapply(dat.icio, function(dat) {
    ...
    dat[
      !dat$country.ind %in% c("VALU", "OUTPUT"),
...
save(tau, file = "./output/params/usa_row_tau.RData")
...
write.csv(prod.all, "./output/params/usa_row_prd_initial.csv", row.names = FALSE)
...
write.csv(prod.usa, "./output/params/usa_prd.csv", row.names = FALSE)
...
write.csv(pwt, file = "./output/cleaned_data/usa_row_pwt.csv", row.names = FALSE)
