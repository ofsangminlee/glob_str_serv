...
## Load trade matrix and the calculated trade costs
...
clean.tau <- function(dat.tau, method = "wedge") {
  if (method == "wedge") {
    ...
    list.r <- setdiff(list.reclass, "bts")
  }
...
plot.boxes.options <- function(bw = TRUE) {
  res <- list(
    theme(axis.text = element_text(size = 13), text = element_text(size = 15), strip.text = element_text(size = 16)) ## If I don't set sizes for axis.text separately, due to the incompleteness of the theme, 15*0.8 (default rel. size) will be the size for axis.text. & strip.text size would affect the size of the subtitle.
  )
  if (bw) {
...
plot.boxes.color <- function(tau.plot, num.ylim) {
  ggplot(tau.plot, aes(x = origin.destination, y = value, fill = origin.destination, pattern = as.factor(year))) +
    ...
    coord_cartesian(ylim = num.ylim)
} # Important note: This does not change the underlying data, unlike ylim(c(0,20)). If you do ylim(c(0,20)), only values within (0,20) will be included for calculation of statistics.

pdf(file = "./doc/figures/tau_box.pdf", width = 10, height = 6)
...
## Trade cost statistics for the developing and advanced countries
...
write.csv(summary.year, "./doc/figures/tau_22.csv", row.names = FALSE)
write.csv(summary.year.hs, "./doc/figures/tau_22_hs.csv", row.names = FALSE)
...
pdf(file = "./doc/figures/tau_22.pdf", width = 10, height = 6)
plot.median.22(summary.plot) + plot.time.options(x.limits = c(1992, 2018), bw = TRUE) + facet_wrap(~title.alp)
dev.off()

pdf(file = "./doc/figures/tau_22_hs.pdf", width = 10, height = 6)
...
## Write down the numbers for China, Vietnam, and Lithuania, for the paper.
temp <- subset(dat.plot.tau.weighted, country %in% c("CHN", "VNM", "LTU") & year %in% c(1995, 2018) & ind %in% c("G", "HTS") & type == "Relative export cost")
temp <- temp[, c("country", "year", "ind", "value")]
temp <- pivot_wider(temp, names_from = "year", values_from = "value")
temp$change <- (temp$`1995` - temp$`2018`) / temp$`1995`

write.csv(temp, "./doc/numbers/rel_tau_chn_vnm_ltu.csv", row.names = FALSE)
...
## Plot (China)
pdf(file = "./doc/figures/tau_cty_chn.pdf", width = 10, height = 5)
plot.one.paper("CHN", adjust.grid.b = TRUE, y.limits.1.b = c(1, 14), y.limits.2.b = c(-0.4, 2.2))
dev.off()

pdf(file = "./doc/figures/tau_cty_chn_hs.pdf", width = 10, height = 5)
...
## Plot (India)
pdf(file = "./doc/figures/tau_cty_ind.pdf", width = 10, height = 5)
plot.one.paper("IND")
dev.off()

## Plot (Vietnam)
pdf(file = "./doc/figures/tau_cty_vnm.pdf", width = 10, height = 5)
plot.one.paper("VNM")
dev.off()
...
write.csv(tau.index, "./output/params/index.csv", row.names = FALSE)
write.csv(tau.index.hs, "./output/params/index_hs.csv", row.names = FALSE)


write.csv(tau.index.alt, "./output/params/index_alt.csv", row.names = FALSE)
write.csv(tau.index.gs, "./output/params/index_gs.csv", row.names = FALSE)
write.csv(tau.index.low, "./output/params/index_low.csv", row.names = FALSE)
write.csv(tau.index.high, "./output/params/index_high.csv", row.names = FALSE)
write.csv(tau.index.orig, "./output/params/index_orig.csv", row.names = FALSE)
...
## Function to preprare data frame for plotting
...
asym.dyn.plot <- function(plot.ind) {
  ggplot(data = subset(plot.ind, ind %in% c("G", "HTS")), aes(x = log(gdppc), y = value)) +
...
pdf(file = "./doc/figures/tau_asymmetry_dynamics.pdf", width = 10, height = 6)
...
asym.dyn.plot(plot.ind.low) + plot.scatter.options(bw = TRUE) +
  facet_wrap(~title.alp)
dev.off()

pdf(file = "./doc/figures/tau_asymmetry_dynamics_high.pdf", width = 10, height = 6)
asym.dyn.plot(plot.ind.high) + plot.scatter.options(bw = TRUE) +
  facet_wrap(~title.alp)
dev.off()
...
pdf(file = "./doc/figures/tau_asymmetry_dynamics_elasticity.pdf", width = 10, height = 6)
