...
## Identification comes from within-country cross-year differences.
...
f.exp <- function(param, x, list.ind.in, base.ind) {
  ...
  if (length(list.ind.from) == 2) {
    x.exo.eps <-
      cbind(
        x.exo * (x[, pasted(list.ind.from[1], base.ind)] - (1 - exp(param[1])) * x[, paste0("rel.p.", list.ind.from[1], ".", base.ind)]),
...
## Recover alpha's through two strategies: 1) fixed effects terms 2) treating them as yearly wedges
list.country <- unique(pro$country)

alphas.year <- do.call("rbind", lapply(years.x, function(i.year) {
  do.call("rbind", lapply(list.country, function(ex.country) {
...
alphas.year.alt <- do.call("rbind", lapply(years.x, function(i.year) {
  do.call("rbind", lapply(list.country, function(ex.country) {
...
alphas.year.gs <- do.call("rbind", lapply(years.x, function(i.year) {
  do.call("rbind", lapply(list.country, function(ex.country) {
...
## Sanity check: check whether countries' production function input shares are well predicted by the estimated parameters.
...
## Sanity check
max(unlist(lapply(years.x, function(x) prod.share.pred.year(x, list.reclass, c("v", list.reclass), "g", add.rel.share(pro, c("v", list.reclass), "g"), sum.rho, alphas.year))))

max(unlist(lapply(years.x, function(x) prod.share.pred.year(x, list.reclass.gs, c("v", list.reclass.gs), "g", add.rel.share(pro.gs, c("v", list.reclass.gs), "g"), sum.rho.gs, alphas.year.gs))))

max(unlist(lapply(years.x, function(x) prod.share.pred.year(x, list.reclass.alt, c("v", list.reclass.alt), "g", add.rel.share(pro.alt, c("v", list.reclass.alt), "g"), sum.rho.alt, alphas.year.alt))))
...
write.csv(sum.rho, "./output/params/rho.csv", row.names = FALSE)
write.csv(sum.rho.alt, "./output/params/rho_alt.csv", row.names = FALSE)
write.csv(sum.rho.gs, "./output/params/rho_gs.csv", row.names = FALSE)
write.csv(sum.rho.usa, "./output/params/rho_wo_usa.csv", row.names = FALSE)
...
write.csv(alphas.country, "./output/params/alphas.csv", row.names = FALSE)
write.csv(alphas.country.alt, "./output/params/alphas_alt.csv", row.names = FALSE)
write.csv(alphas.country.gs, "./output/params/alphas_gs.csv", row.names = FALSE)
...
## Save unit costs
write.csv(uc, "./output/params/unit_costs.csv", row.names = FALSE)

write.csv(uc.alt, "./output/params/unit_costs_alt.csv", row.names = FALSE)

write.csv(uc.gs, "./output/params/unit_costs_gs.csv", row.names = FALSE)
...
## Wedges version
alphas.inner.t <- do.call("rbind", lapply(years.x, function(i.year) {
  do.call("rbind", lapply(list.country, function(ex.country) {
...
alphas.outer.t <- do.call("rbind", lapply(years.x, function(i.year) {
  do.call("rbind", lapply(list.country, function(ex.country) {
...
write.csv(sum.rho.inner, "./output/params/rho_nest_inner.csv", row.names = FALSE)

sum.rho.outer.t$method <- "year"
sum.rho.outer.a$method <- "average"
sum.rho.outer <- rbind(sum.rho.outer.t, sum.rho.outer.a)

write.csv(sum.rho.outer, "./output/params/rho_nest_outer.csv", row.names = FALSE)
...
write.csv(alphas.country.nest, "./output/params/alphas_nest.csv", row.names = FALSE)

uc.nest <- rbind(pro.outer.t, pro.outer.a)

write.csv(uc.nest, "./output/params/unit_costs_nest.csv", row.names = FALSE)

## Note: the estimated production functions in Atalay (2017) and Cravino and Sotelo (2019) are close to Leontieff. Why? US data.
...
write.csv(sum.rho.inner.usa, "./output/params/rho_nest_inner_only_usa.csv", row.names = FALSE)
