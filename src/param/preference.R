...
## Remark 2: identification is coming mostly from with-in country cross-year differences.
## Remark 3: If one country's relative income or prices are very stable across time, this will cause collinearity issue with that country's fixed effect terms. This will make identification weak and standard error blow up. The prominent example is Brunei. This is the reason behind bootstrapping.
...
## In the estimating equation, what matters are relative prices and relative income. Function to get relative prices.
...
Dg.exp <- function(param, x) {
  G <- matrix(
    c(
      mean(exp(param[1]) * x[, "p.bts"] * (x[, "p.bts"] + (exp(param[2]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "pc.total"] * (x[, "p.bts"] + (exp(param[2]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "p.hts"] * (x[, "p.hts"] + (exp(param[3]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "pc.total"] * (x[, "p.hts"] + (exp(param[3]) - 1) * x[, "pc.total"])), # exp(param[1])'s derivatives
...
Dg.exp.alt <- function(param, x) {
  G <- matrix(
    c(
      mean(exp(param[1]) * x[, "p.cs"] * (x[, "p.cs"] + (exp(param[2]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "pc.total"] * (x[, "p.cs"] + (exp(param[2]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "p.ps"] * (x[, "p.ps"] + (exp(param[3]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "pc.total"] * (x[, "p.ps"] + (exp(param[3]) - 1) * x[, "pc.total"])), # exp(param[1])'s derivatives
...
Dg.exp.gs <- function(param, x) {
  G <- matrix(
    c(
      mean(exp(param[1]) * x[, "p.s"] * (x[, "p.s"] + (exp(param[2]) - 1) * x[, "pc.total"])), mean(exp(param[1]) * x[, "pc.total"] * (x[, "p.s"] + (exp(param[2]) - 1) * x[, "pc.total"])),
...
res.gs <- add.g(res.gs) ## Remark: EPS.S is greater than both EPS.HTS and EPS.BTS. Why? Compared to the three-sector baseline, merging HTS and BTS implies that now relative price of S is increasing slower than BTS and faster than HTS. Therefore, it is possible that the EPS.S is higher than either EPS.HTS or EPS.BTS.
...
## Recovering phi's
## To obtain the sectoral weights, there are three possible ways: 1. average across years, 2. wedges approach, and 3. first-year matching (2 nests 3 in that 3 is the first year of 2.)
...
## ## It is possible to print results for each country: phi's are pretty stable.
## ex.c <- "IND"
...
## Sanity check of whether the consumption shares are accurately predicted.
...
## If the below shows value close to zero. It means that the parameters estimated using the demand system yields the demand that was the input of estimation. For the two steps, I used two different representations of the demand system.
...
## Save the result
write.csv(res, "./output/params/sub_inc_elasticity.csv", row.names = FALSE)

write.csv(res.alt, "./output/params/sub_inc_elasticity_alt.csv", row.names = FALSE)

write.csv(res.gs, "./output/params/sub_inc_elasticity_gs.csv", row.names = FALSE)

write.csv(res.usa, "./output/params/sub_inc_elasticity_wo_usa.csv", row.names = FALSE)
...
write.csv(phis.country, "./output/params/weight_util.csv", row.names = FALSE)

write.csv(phis.country.alt, "./output/params/weight_util_alt.csv", row.names = FALSE)

write.csv(phis.country.gs, "./output/params/weight_util_gs.csv", row.names = FALSE)

## Note: for the US-ROW case, I will back out \phi's to match the initial year for USA-ROW cases.
