...
function model_inner(W; N, S, L, Ā, τ, nx, σ, ϵ, ϕ, η, ρ, α, θ, ind_us, P_start)
    ...
    # Derive sectoral net exports
    NX_sec = Array{Float64}(undef, N, S)
    for i in 1:N
        for k in 1:S
            NX_sec[i,k] = sum(sh[i,:,k].*P_sec[:,k].*Q_sec[:,k]) - P_sec[i,k]*Q_sec[i,k]
        end
    end

    # return((;Z = Z, P_f = P_f, C_f = C_f, P_sec = P_sec, R = R, Q_sec = Q_sec, C_sec = C_sec, M_sec = M_sec, L_sec = L_sec, sh = sh))
    # One change for JIE revision: input-output matrix needs to be in the output.
    return(DataFrame(Z = [Z], P_f = [P_f], C_f = [C_f], P_sec = [P_sec], R = [R], Q_sec = [Q_sec], C_sec = [C_sec], M_sec = [M_sec], L_sec = [L_sec], sh = [sh], ψ = [ψ], NX_sec = [NX_sec]))
end
