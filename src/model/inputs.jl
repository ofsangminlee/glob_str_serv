...
function get_pref(file_name, list_eps)

    params = DataFrame(CSV.File("./output/params/" * file_name))
...
(σ, ϵ) = get_pref("sub_inc_elasticity.csv", string.("eps.", list_inds))

(σ_alt, ϵ_alt) = get_pref("sub_inc_elasticity_alt.csv", string.("eps.", list_inds_alt))

(σ_gs, ϵ_gs) = get_pref("sub_inc_elasticity_gs.csv", string.("eps.", list_inds_gs))

function get_phis(file_name, list_inds; method="avg")
    phis = DataFrame(CSV.File("./output/params/" * file_name))
...
ϕ = get_phis("weight_util.csv", list_inds)
ϕ_wedge = get_phis("weight_util.csv", list_inds, method="wedge")
ϕ_alt = get_phis("weight_util_alt.csv", list_inds_alt)
ϕ_gs = get_phis("weight_util_gs.csv", list_inds_gs)

# 2. Production parameters
η = [2.0, 2, 2]
η_gs = [2.0, 2]

function get_rho(file_name, list_inds)
    rhos = DataFrame(CSV.File("./output/params/" * file_name))
    rhos = rhos[indexin(rhos.ind, list_inds), :]
    ρ = Array(rhos.rho)
    return (ρ)
end

ρ = get_rho("rho.csv", list_inds)
ρ_alt = get_rho("rho_alt.csv", list_inds_alt)
ρ_gs = get_rho("rho_gs.csv", list_inds_gs)

ρ_inner = get_rho("rho_nest_inner.csv", list_inds)

temp = DataFrame(CSV.File("./output/params/rho_nest_outer.csv"))
...
function get_alphas(file_name, list_inds; method="avg")
    alphas = DataFrame(CSV.File("./output/params/" * file_name))
...
α = get_alphas("alphas.csv", list_inds)
α_wedge = get_alphas("alphas.csv", list_inds, method="wedge")
α_alt = get_alphas("alphas_alt.csv", list_inds_alt)
α_gs = get_alphas("alphas_gs.csv", list_inds_gs)

α_nest = get_alphas("alphas_nest.csv", list_inds)
α_inner = α_nest[:, 1:3, :]
α_outer = α_nest[:, 4, :]
...
function get_tau(file_name, name_dataframe, list_inds, list_countries)
    tau = load("./output/params/" * file_name)[name_dataframe]
    S = length(list_inds)
    τ_seq = []
...
τ = get_tau("tau.RData", "tau", list_inds, list_countries)
τ_hs = get_tau("tau_head_ries.RData", "tau.hs", list_inds, list_countries)

τ_orig = get_tau("tau_orig.RData", "tau.orig", list_inds, list_countries)

τ_alt = get_tau("tau_alt.RData", "tau.alt", list_inds_alt, list_countries)
τ_gs = get_tau("tau_gs.RData", "tau.gs", list_inds_gs, list_countries)
τ_low = get_tau("tau_low.RData", "tau.low", list_inds, list_countries)
τ_high = get_tau("tau_high.RData", "tau.high", list_inds, list_countries)

# 3. Productivities

# Function to get productivities
function get_prd(file_name, list_inds)
    prd = DataFrame(CSV.File("./output/params/" * file_name))
...
A = get_prd("prd.csv", list_inds)
A_alt = get_prd("prd_alt.csv", list_inds_alt)
A_gs = get_prd("prd_gs.csv", list_inds_gs)
A_low = get_prd("prd_low.csv", list_inds)
A_high = get_prd("prd_high.csv", list_inds)
A_wedge = get_prd("prd_wedge.csv", list_inds)
A_nest = get_prd("prd_nest.csv", list_inds)
A_no_s_trade = get_prd("prd_no_s_trade.csv", list_inds)

## Get the productivity for the interaction (spillover) case.
temp = DataFrame(CSV.File("./output/params/prd_tau_interaction.csv"))
sort!(temp, [:year, :country])
A_interaction = []

for year in 1995:2018
    prd_temp = temp[temp.year.==year, :]
    A_temp = Array{Float64}(undef, N, S)
    for (i, ind) in enumerate(list_inds)
        if ind == "g"
            A_temp[:, i] .= prd_temp[prd_temp.ind.==ind, "prd.count"]
        else
            A_temp[:, i] .= prd_temp[prd_temp.ind.==ind, "prd"]
        end
    end
    global A_interaction = [A_interaction; [A_temp]]
end

# 4. NX terms (nominal NX)
NX_data = DataFrame(CSV.File("./output/params/nx.csv"))
...
function get_p_start(file_name, list_inds)
    p_data = DataFrame(CSV.File("./output/cleaned_data/" * file_name))
...
p_start = get_p_start("gopl.csv", list_inds)
p_start_alt = get_p_start("gopl_alt.csv", list_inds_alt)
p_start_gs = get_p_start("gopl_gs.csv", list_inds_gs)
...
model_input = DataFrame(N=[N], S=[S], S_gs=[S_gs],
    ...
    A=[A], A_wedge=[A_wedge], A_alt=[A_alt], A_gs=[A_gs], A_low=[A_low], A_high=[A_high], A_nest=[A_nest], A_no_s_trade=[A_no_s_trade], A_interaction=[A_interaction],
    L=[L],
