## clean_result.jl
# From the model simulations (baseline and counterfactual equilibria), derive structural transformation pattern
...
# Baseline
str_chg = vcat(result(m_data, "data", list_inds),
    result(m_base, "baseline", list_inds),
    result(m_only_g, "count_only_g", list_inds),
    result(m_only_s, "count_only_s", list_inds),
    result(m_no, "count_no", list_inds))

# Head and Ries case
str_chg_hs = vcat(result(m_hs, "baseline", list_inds),
    result(m_only_g_hs, "count_only_g", list_inds),
    result(m_only_s_hs, "count_only_s", list_inds),
    result(m_no_hs, "count_no", list_inds))

# Model with non-tradable services
str_chg_no_s_trade = vcat(result(m_no_s_tr, "baseline", list_inds),
    result(m_no_s_tr_no, "count_no", list_inds))

# Model with productivities and trade costs interactions.
str_chg_interaction = vcat(result(m_interaction, "baseline", list_inds),
    result(m_interaction_no, "count_no", list_inds))
...
str_chg_gs = get_result(m_gs, m_gs_no, list_inds_gs)

# 2. Alternative three sector
str_chg_alt = get_result(m_alt, m_alt_no, list_inds_alt)

# 3. Trade elasticity for services
# Low
str_chg_low = get_result(m_low, m_low_no, list_inds)

# High
str_chg_high = get_result(m_high, m_high_no, list_inds)

# 4. Wedges approach
str_chg_wedge = get_result(m_wedge, m_wedge_no, list_inds)

# 5. Nested production function
str_chg_nest = get_result(m_nest, m_nest_no, list_inds)

# 6. No global imbalances (NX = 0)
str_chg_nx = get_result(m_nx, m_nx_no, list_inds)

# 7. Non-tradable CS case
str_chg_bts = get_result(m_bts, m_bts_no, list_inds)

# 8. No tau < 1 adjustment case
str_chg_orig = get_result(m_orig, m_orig_no, list_inds)

# Save the result in CSV file
# CSV.write("../../output/model/str_chg.csv", str_chg)

for i_model in ["str_chg", "str_chg_hs", "str_chg_no_s_trade", "str_chg_interaction", "str_chg_gs", "str_chg_alt", "str_chg_low", "str_chg_high", "str_chg_wedge", "str_chg_nest", "str_chg_nx", "str_chg_bts", "str_chg_orig"]
...
relp_data = get_relp(m_data, list_inds)
relp_data.type = repeat(["data"], nrow(relp_data))

relp_base = get_relp(m_base, list_inds)
...
sh_data = clean_sh(m_data, list_inds)
sh_data[!, "type"] .= "data"
sh_base = clean_sh(m_base, list_inds)
...
lprd_base = lprd(m_base, "baseline", list_inds)
