...
## Productivities (TFP and Labor Productivity)
# Note: for labor productivity, we need to obtain baseline equilibrium first.
...
# TFP
a <- read.csv("./output/params/prd.csv")
...
pdf(file = "./doc/figures/prd_tfp.pdf", width = 10, height = 6)
plot.prd(c.a.paper, y.lab = "Quartiles (TFP)", type.title = "title.rom")
dev.off()

pdf(file = "./doc/figures/prd_lp.pdf", width = 10, height = 6)
plot.prd(c.la.paper, y.lab = "Quartiles (LP)", type.title = "title.rom")
dev.off()

pdf(file = "./doc/figures/prd_tfp_gr.pdf", width = 10, height = 6)
...
pdf(file = "./doc/figures/prd_price_gr.pdf", width = 10, height = 6)
plot.relp(sum.g.relp, y.lab = "Quartiles (growth factor of relative prices)", type.title = "title.rom")
dev.off()

# Count the countries for illustration in the paper, where prices follow the usual pattern.
