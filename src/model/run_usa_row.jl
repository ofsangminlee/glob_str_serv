...
using RData, DataFrames, JLD2, CSV, SpecialFunctions, Optim, LinearAlgebra, Statistics, StatsBase
...
params = DataFrame(CSV.File("./output/params/sub_inc_elasticity_wo_usa.csv"))
...
phis = DataFrame(CSV.File("./output/params/usa_row_weight_util.csv"))
...
rhos = DataFrame(CSV.File("./output/params/rho_wo_usa.csv"))
rhos.ind == ["g", "bts", "hts"]
ρ = Array(rhos.rho)

# Fixed α across years
alphas = DataFrame(CSV.File("./output/params/usa_row_alphas.csv"))
...
# Labor endowment
labor = DataFrame(CSV.File("./output/cleaned_data/usa_row_pwt.csv"))
sort!(labor, :country)
L_seq = []
...
# Trade costs
tau = load("./output/params/usa_row_tau.RData")["tau"]
...
# Productivities
product = DataFrame(CSV.File("./output/params/usa_row_prd_initial.csv"))
...
product_usa = DataFrame(CSV.File("./output/params/usa_prd.csv"))
...
fy_check.P_sec[1] # Prices, by assumption, should be equalized across countries.
temp = fy_check.L_sec[1]
temp./sum(temp, dims = 2) # In the data, USA first-year GDP share is (22, 39, 39%) for (G, BTS, HTS)
...
@save "./output/model/usa_row_a.jl2" A_usa_zzz_seq # Save the calculated productivities. However we are not using it for other purposes.
