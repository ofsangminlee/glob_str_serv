...
add.title <- function(dat) {
  ...
  # Add sentence case for JIE style

  dat$title.alp.stnc <- ifelse(dat$ind == "G", "(a) Goods",
    ifelse(dat$ind == "HTS", "(b) Highly tradable services",
      ifelse(dat$ind == "BTS", "(c) Barely tradable services", "")
    )
  )
  dat$title.alp.stnc <- factor(dat$title.alp.stnc, levels = c("(a) Goods", "(b) Highly tradable services", "(c) Barely tradable services"))
...
# Adding a super title for the journal figure
add_main_title <- function(plot_obj, title_text, title_size = 16, rel_heights = c(0.1, 1)) {
  # Create the title object
  title_header <- ggdraw() +
    draw_label(
      title_text,
      fontface = "plain", # One can do "bold"
      x = 0.5,
      hjust = 0.5,
      size = title_size
    )

  # specific relative heights for title vs plot
  return(plot_grid(title_header, plot_obj, ncol = 1, rel_heights = rel_heights))
}
